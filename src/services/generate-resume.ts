...
async function runResumeGeneration(job: typeof jobs.$inferSelect) {
  ...
  const { indexGeneratedFile } = await import('@/services/memory');
  indexGeneratedFile(`${job.id}-resume`, 'resume.md', result.updatedResume, 'text/markdown');
  indexGeneratedFile(`${job.id}-cover-letter`, 'cover-letter.md', result.coverLetter, 'text/markdown');
